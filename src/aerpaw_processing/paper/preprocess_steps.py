...
from aerpaw_processing.paper.preprocess_utils import StepEnum, add_step_entry, get_step_entry, get_col_tech_name, get_tech_independent_cols, get_alias_map, get_col_categorical_map, get_env_var
...
def rename_columns(context: pd.DataFrame):
    ...
    working: pd.DataFrame = get_step_entry(StepEnum.READ_DATA, context)

    alias_map = get_alias_map()
    working["data"] = working["data"].apply(
        lambda df: df.rename(columns=alias_map)
    )

    add_step_entry(StepEnum.RENAME_COLUMNS, working, context)

def remove_columns(context: pd.DataFrame):
    """Remove extra columns that aren't defined in CONFIG"""

    working: pd.DataFrame = get_step_entry(StepEnum.RENAME_COLUMNS, context)
    ...
    add_step_entry(StepEnum.REMOVE_COLUMNS, working, context)

def _combine_dfs(dfs: list[pd.DataFrame], combine_column: str, log_info: str) -> pd.DataFrame:
    ...
    for i in range(1, len(dfs)):
        ...
        cols_to_drop = [c for c in next_df.columns if c in combined_df.columns and c != combine_column]
        combined_df = pd.merge(combined_df, next_df.drop(columns=cols_to_drop), on=combine_column, how="outer")
        
    return combined_df

def combine_tech_files(context: pd.DataFrame, remove_cols: bool, combine_column: str = TIMESTAMP_COL):
    ...
    latest = StepEnum.REMOVE_COLUMNS if remove_cols else StepEnum.RENAME_COLUMNS 

    working: pd.DataFrame = get_step_entry(latest, context)
    ...
    add_step_entry(StepEnum.COMBINE_TECH_FILES, working_combined, context)


def combine_flight_techs(context: pd.DataFrame, combine_column: str = TIMESTAMP_COL):
    ...
    working: pd.DataFrame = get_step_entry(StepEnum.COMBINE_TECH_FILES, context)
    ...
    working_combined = pd.DataFrame(combined_rows)
    add_step_entry(StepEnum.COMBINE_FLIGHT_TECHS, working_combined, context)


def interpolate_to_label(context: pd.DataFrame, label_col: str):
    ...
    working: pd.DataFrame = get_step_entry(StepEnum.COMBINE_FLIGHT_TECHS, context)
    cat_map = get_col_categorical_map()
    
    # 1. Identify multiple label columns
    label_variants = [label_col] + [get_col_tech_name(label_col, tech) for tech in TECH_LIST]
    ...
    for _, row in working.iterrows():
        ...
        if not actual_labels:
            logger.warning(f"No label columns {label_variants} found for flight {row['flight_name']}")
            new_rows.append({
    ...
    add_step_entry(StepEnum.INTERPOLATE_TO_LABEL, working_interpolated, context)


def project_coordinates(context: pd.DataFrame):
    ...
    working: pd.DataFrame = get_step_entry(StepEnum.INTERPOLATE_TO_LABEL, context)
    ...
    for _, row in working.iterrows():
        ...
        if "Latitude" in df.columns and "Longitude" in df.columns:
            # pyproj.Proj expects (longitude, latitude)
            # x, y are returned in meters
            x, y = local_proj(df["Longitude"].values, df["Latitude"].values)
            df["x"] = x
            df["y"] = y
            
        if "Altitude" in df.columns:
            # Altitude is assumed to be in meters relative to launch point (0m)
            df["z"] = df["Altitude"]
    ...
    add_step_entry(StepEnum.PROJECT_COORDINATES, working_projected, context)



def calculate_angular_bin(context: pd.DataFrame):
...
def calculate_correlation(context: pd.DataFrame, label_col: str = LABEL_COL):
    ...
    for _, row in working.iterrows():
        ...
        if actual_label is None or 'bin' not in df.columns or 'x' not in df.columns:
            logger.warning(f"Missing required columns for correlation computation in flight {row['flight_name']}")
            new_rows.append({
                "dataset_id": row["dataset_id"],
                "flight_name": row["flight_name"],
        ...
        valid_df['d3D'] = np.sqrt(valid_df['x']**2 + valid_df['y']**2 + (valid_df['z'] - 10.0)**2)
        ...
        if corr_data:
            ...
            flight_corr_df['radial_bin'] = np.floor(flight_corr_df['dist'] / RADIAL_BIN_SIZE) * RADIAL_BIN_SIZE
            
            # Now we aggregate over all angular bins to generate a composite RSRP correlation profile
            # as a function of radial distance
            aggregated_corr = cast(pd.Series, flight_corr_df.groupby('radial_bin')['r'].mean()).reset_index()
            aggregated_corr['num_pairs'] = flight_corr_df.groupby('radial_bin').size().values
            
            bin_stats = flight_corr_df.groupby('bin').agg({'r': 'mean', 'dist': 'max'}).reset_index()
            ...
            new_rows.append({
             ...
             new_rows.append({ "dataset_id": row["dataset_id"],
                "flight_name": row["flight_name"],
                "data": pd.DataFrame(),
...
def calculate_fast_fading_correlation(context: pd.DataFrame, label_col: str = LABEL_COL):
    ...
    for _, row in working.iterrows():
        ...
        if actual_label is None or 'x' not in df.columns:
            logger.warning(f"Missing required columns for fast fading correlation in flight {row['flight_name']}")
            new_rows.append({
                "dataset_id": row["dataset_id"],
                "flight_name": row["flight_name"],
                "data": pd.DataFrame()
        ...
        d3D_bs = np.sqrt(valid_df['x']**2 + valid_df['y']**2 + (valid_df['z'] - 10.0)**2)
        ...
        if r_list:
            ...
            ff_df['spatial_bin'] = np.floor(ff_df['dist'] / SPATIAL_BIN_SIZE) * SPATIAL_BIN_SIZE
            ...
            aggregated_ff = ff_df.groupby('spatial_bin').agg(
                r=('r', 'mean'),
                num_pairs=('r', 'count')
            ).reset_index()
...
def save_datasets(context: pd.DataFrame):
    cleaned_home = Path(get_env_var("DATASET_CLEAN_HOME"))
    working = get_step_entry(StepEnum.PROJECT_COORDINATES, context)

    
    for _, row in working.iterrows():
        cast(pd.DataFrame, row['data']).to_csv(cleaned_home / f"Dataset_{row['dataset_id']}_{cast(str, row['flight_name']).replace(" ", "_")}.csv")
...
def run(remove_cols: bool = REMOVE_COL, label_col: str = LABEL_COL):
    context = pd.DataFrame(columns=['step', 'step_data'])
    
    read_data(context)

    rename_columns(context)

    if remove_cols:
        remove_columns(context)

    combine_tech_files(context, remove_cols)

    combine_flight_techs(context)

    interpolate_to_label(context, label_col)

    project_coordinates(context)

    calculate_angular_bin(context)

    calculate_correlation(context, label_col)

    calculate_fast_fading_correlation(context, label_col)

    save_datasets(context)
