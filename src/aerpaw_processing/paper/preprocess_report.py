...
from aerpaw_processing.paper.preprocess_utils import StepEnum, get_label_col, get_step_entry, get_env_var
...
def _generate_3d_plot(step_df: pd.DataFrame, report_dir: Path, step_name: str, label: str, title_suffix: str, report: list, is_categorical: bool = False):
        ...
        if 'x' in df.columns and 'y' in df.columns and 'z' in df.columns:
            ...
            if not plot_df.empty:
                ...
                fig = px.scatter_3d(
                    ...
                    color_discrete_sequence=px.colors.qualitative.Alphabet if is_categorical else None,
                    hover_data=[rsrp_col]
                )
                ...
                if step_name == "bin":
                    ...
                    if 'bin' in plot_df.columns and not plot_df.empty:
                        ...
                        dist_sq = (plot_df['x'] - mid_x)**2 + (plot_df['y'] - mid_y)**2
                        ...
                        for i, (b_val, name_desc) in enumerate(zip([bin1, bin2, bin3], names)):
                            if b_val not in seen_bins:
                                seen_bins.add(b_val)
                                bins_to_plot.append({"bin_id": b_val, "color": colors[i], "name": f"Bin: {name_desc}"})
                        ...
                        for b_info in bins_to_plot:
                            ...
                            r_max = np.sqrt(b_df['x']**2 + b_df['y']**2 + (b_df['z'] - 10)**2).max()
                            ...
                            for idx, (t, p) in enumerate(rays):
                                fig.add_trace(go.Scatter3d(
                                    ...
                                    name=b_info["name"] if idx == 0 else f"{b_info['name']} line {idx+1}",
                                    showlegend=(idx == 0)
                                ))
                ...
                try:
                    ...
                    logger.warning(f"Could not save static image for {flight_name}: {e}. Ensure 'kaleido' is installed.")
                    rel_html = f"report_assets/{html_filename}"
                    report.append(f"#### Flight: {flight_name}\n")
...
def report_read_data(step_df: pd.DataFrame, report: list[str]):
    ...
    for _, r in cast(pd.DataFrame, step_df).iterrows():
        df = r['data']
        report.append(f"| {r['dataset_id']} | {r['tech']} | {r['flight_name']} | {r['filepath']} | {df.shape[0]:,} | {df.shape[1]} |")
    report.append("\n")

def report_rename_columns(context: pd.DataFrame, step_df: pd.DataFrame, report: list[str]):
    ...
    for i in range(len(step_df)):
        ...
        changed_count = sum(1 for old, new in zip(old_df.columns, new_df.columns) if old != new)
        
        r = cast(pd.DataFrame, step_df).iloc[i]
        report.append(f"| {r['dataset_id']} | {r['tech']} | {r['flight_name']} | {r['filepath']} | {changed_count} |")
...
def report_remove_columns(context: pd.DataFrame, step_df: pd.DataFrame, report: list[str]):
    ...
    report.append("| Dataset | Tech | Flight | Filepath | Cols Removed | Remaining Columns |")
    report.append("| --- | --- | --- | --- | --- | --- |")
    ...
    for i in range(len(step_df)):
        ...
        r = cast(pd.DataFrame, step_df).iloc[i]
        report.append(f"| {r['dataset_id']} | {r['tech']} | {r['flight_name']} | {r['filepath']} | {removed_count} | `{remaining_cols}` |")
    report.append("\n")

def report_combine_tech_files(context: pd.DataFrame, step_df: pd.DataFrame, report: list[str]):
    
    prev_step = StepEnum.REMOVE_COLUMNS if StepEnum.REMOVE_COLUMNS.value in context["step"].values else StepEnum.RENAME_COLUMNS
    prev_step_df = get_step_entry(prev_step, context)

    num_combined = len(step_df)
    report.append(f"- Multiple KPI files combined into {num_combined} technology-specific datasets per flight.")
    report.append("\n### Combined Datasets\n")
    ...
    for _, r in cast(pd.DataFrame, step_df).iterrows():
        ...
        report.append(f"| {r['dataset_id']} | {r['tech']} | {r['flight_name']} | {df.shape[0]:,} | {df.shape[1]} | {prev_rows_str} | `{cols_str}`")

    report.append("\n")


def generate_report(context: pd.DataFrame):
    """Generate a markdown report from the processing context."""

    context_home = Path(get_env_var("CONTEXT_HOME"))
    report_home = context_home / "report/"
    ...
    for _, row in context.iterrows():
        ...
        if step_val == StepEnum.READ_DATA.value:
            ...
            num_combined = len(step_df)
            report.append(f"- Multiple technologies combined into {num_combined} single flight datasets.")
            ...
            for _, r in cast(pd.DataFrame, step_df).iterrows():
                ...
                report.append(f"| {r['dataset_id']} | {r['flight_name']} | {df.shape[0]:,} | {df.shape[1]} | {techs} | `{cols_str}`")
            ...
            for _, r in cast(pd.DataFrame, step_df).iterrows():
                ...
                prev_rows = matched_prev.iloc[0]['data'].shape[0] if not matched_prev.empty else 0
                report.append(f"| {r['dataset_id']} | {r['flight_name']} | {df.shape[0]:,} | {df.shape[1]} | {prev_rows:,} |")
            report.append("\n")

        elif step_val == StepEnum.PROJECT_COORDINATES.value:
            report.append("- Projected Latitude/Longitude to x/y coordinates using AEQD projection.")
            report.append(f"- Base Tower (Origin): {BASE_TOWER.name} ({BASE_TOWER.lat}, {BASE_TOWER.lon})")
            ...
            report.append("- Partitioned the 3D data points into a spherical grid centered at the Base Station with 0.05 rad bins.")
            report.append("\n### Projected 3D Trajectories (Colored by Angular Bin)\n")
            
            _generate_3d_plot(step_df, report_home, "bin", "bin", " (Angular Bins)", report, is_categorical=True)
            report.append("\n")

        elif step_val == StepEnum.CORRELATION_COMPUTATION.value:
            report.append("- Computed pairwise spatial correlation of RSRP for each angular bin and aggregated by 5m radial separation distance bins.")
            
            report.append("\n**Note on Pairwise Point Generation:**")
            report.append("For a bin with `N` points, the correlation is computed between every point and every other point in that *same* bin.")
            report.append("Because we pair every point with every other point, the total number of generated correlation pairs will be much larger than the original data points (Quadratic Growth).")
            report.append("\n```text")
            ...
            for _, r in cast(pd.DataFrame, step_df).iterrows():
                ...
                fig.add_trace(
                    go.Bar(x=df['radial_bin'], y=df['num_pairs'], name="Num Points", opacity=0.3, marker_color='red'),
                    secondary_y=True,
                )
                
                fig.add_trace(
                    go.Scatter(x=df['radial_bin'], y=df['r'], name="Avg Correlation", mode='lines+markers', line=dict(color='blue')),
                    secondary_y=False,
                )
                ...
                fig.update_yaxes(title_text="Avg Correlation", secondary_y=False, range=[-1.0, 1.0])
                fig.update_yaxes(title_text="Num Points", secondary_y=True, showgrid=False)
                ...
                if bin_stats is not None and not bin_stats.empty:
                    ...
                    top_dist_bins = bin_stats.sort_values('dist', ascending=False).head(2)['bin'].tolist()
                    
                    flight_points_row = calc_bin_df[(calc_bin_df['dataset_id'] == dataset_id) & (calc_bin_df['flight_name'] == flight_name)]
                    if not flight_points_row.empty:
                        plot_df = flight_points_row.iloc[0]['data']
                        if 'x' in plot_df.columns and 'y' in plot_df.columns and 'z' in plot_df.columns and 'bin' in plot_df.columns:
                            plot_df_clean = plot_df.dropna(subset=['x', 'y', 'z', 'bin']).copy()
                            plot_df_clean['bin'] = plot_df_clean['bin'].astype(str)

                            rsrp_col = get_label_col(plot_df_clean)
                            
                            fig3d = px.scatter_3d(
                                plot_df_clean, x='x', y='y', z='z',
                                color='bin',
                            ...
                            seen_bins = set()
                            colors = ["yellow", "cyan", "magenta"]
                            names = ["Highest Correlation", "Greatest Distance 1", "Greatest Distance 2"]
                            
                            bins_list = [str(top_corr_bin)] + [str(b) for b in top_dist_bins]
                            
                            for i, (b_val, name_desc) in enumerate(zip(bins_list, names)):
                                if b_val not in seen_bins:
                                    seen_bins.add(b_val)
                                    bins_to_plot.append({"bin_id": b_val, "color": colors[i], "name": f"Bin: {name_desc}"})
                            ...
                            base_name3d = f"dataset_{dataset_id}_{safe_flight_name}_bin_corr_3d"
                            ...
                            try:
                                ...
                                report.append(f"#### Flight: {flight_name} 3D Correlation Bins\n")
                                rel_png3d = f"report_assets/{png_filename3d}"
                                rel_html3d = f"report_assets/{html_filename3d}"
                                report.append(f"[![{flight_name} 3D]({rel_png3d})]({rel_html3d})\n")
                            except Exception as e:
                                logger.warning(f"Could not save static image for {flight_name} 3D: {e}. Ensure 'kaleido' is installed.")
                                rel_html3d = f"report_assets/{html_filename3d}"
                                report.append(f"#### Flight: {flight_name} 3D Correlation Bins\n")
                                report.append(f"[Interactive 3D Plot]({rel_html3d})\n")

        elif step_val == StepEnum.FAST_FADING_CORRELATION.value:
            report.append("- Computed fast fading factor correlation in spatio-temporal domain (Section II.B.2) using a rolling window of 5 prior samples.")
            report.append("- Fast fading `nu` estimated by removing LoS path loss (proportional to -20log10(d3D)) from RSRP.")
            ...
            for _, r in cast(pd.DataFrame, step_df).iterrows():
                ...
                fig.add_trace(
                    go.Bar(x=df['spatial_bin'], y=df['num_pairs'], name="Num Points", opacity=0.3, marker_color='green'),
                    secondary_y=True,
                )
                
                fig.add_trace(
                    go.Scatter(x=df['spatial_bin'], y=df['r'], name="Avg Correlation", mode='lines+markers', line=dict(color='orange')),
                    secondary_y=False,
                )
